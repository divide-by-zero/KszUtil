...
namespace KszUtil.MyStorage
{
    ...
    /// ストレージ管理クラス
    /// </summary>
    public class EncryptMyStorage : IMyStorage
    {
        /// <summary>
        /// コンストラクタで指定した時点のフォルダ名しか扱わない。　それよりも下位または上位は対象としない
        /// </summary>
        /// <param name="path"></param>
        public EncryptMyStorage(string path)
        {
        ...
        private Stream OpenRead(DatName datName)
        {
            ...
            {
                return new BrotliStream(File.OpenRead(fullPath), CompressionMode.Decompress);
            }
        ...
        {
            return new BrotliStream(File.Open(FullPath(datName), FileMode.Create), CompressionMode.Compress);
        }
